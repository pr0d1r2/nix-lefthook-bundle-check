...
@test "pre-commit calls bundle check" {
    cmd=$(run_cmd)
    echo "$cmd" | grep -q "bundle check"
}

@test "pre-commit includes --gemfile=Gemfile flag" {
    cmd=$(run_cmd)
    echo "$cmd" | grep -q "\-\-gemfile=Gemfile"
}
...
@test "pre-commit executes bundle check" {
    touch "$WORK/Gemfile"
    ...
    run cat "$STUB_LOG"
    assert_output --partial "bundle check"
